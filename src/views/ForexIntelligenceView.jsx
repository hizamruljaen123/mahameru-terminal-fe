...
export default function ForexIntelligenceView(props) {
  ...
  const buildForexQuery = (symbol, name) => {
    const raw = symbol.replace('=X', '').replace('=F', '');
    const cur1 = raw.slice(0, 3).toUpperCase();
    const cur2 = raw.slice(3, 6).toUpperCase();

    // Map common codes to country names for better regional search
    const codeMap = {
      'IDR': 'Indonesia', 'KRW': 'Korea', 'USD': 'USA', 'EUR': 'Europe',
      'JPY': 'Japan', 'GBP': 'UK', 'AUD': 'Australia', 'CAD': 'Canada',
      'CHF': 'Switzerland', 'CNY': 'China', 'HKD': 'Hong Kong', 'SGD': 'Singapore'
    };

    const country1 = codeMap[cur1] || '';
    const country2 = codeMap[cur2] || '';

    // Specialized query for regional and economic context (matching user preferred RSS query style)
    if (cur1 && cur2 && cur1 !== cur2) {
      return `${country1} ${country2} ${cur1} ${cur2} currency economy exchange rate monetary policy interest rate`.trim();
    }
    const parts = name.split(' / ');
    return `${parts[0] || name} ${parts[1] || ''} economy trade currency market interest rate`.trim();
  };
  ...
  onMount(() => {
    ...
    const syncItv = setInterval(() => fetchPairsList(), 60000);
    solidOnCleanup(() => {
      _mounted = false;
      ...
      <aside class="w-64 border-r border-border_main bg-black/20 flex flex-col shrink-0">
            ...
            {(pair) => (
              ...
              >
                <div class="flex items-center gap-3 flex-1 min-w-0">
                  <div class="w-8 h-8 shrink-0 flex items-center justify-center bg-black/40 border border-border_main text-[10px] font-black group-hover:border-text_accent/40 transition-colors">
                    {pair.name.split(' / ')[0]}
                  </div>
        ...
        <div class="p-3 bg-bg_header/20 border-t border-border_main flex items-center justify-between">
          <div class="flex items-center gap-2">
            <div class="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.8)]"></div>
          ...
          <Show when={detail()} fallback={<div class="animate-pulse flex gap-6"><div class="w-12 h-12 bg-white/5 rounded-sm"></div><div class="w-48 h-10 bg-white/5 rounded-sm"></div></div>}>
            <div class="flex items-center gap-6">
              ...
              <div class="flex flex-col">
                ...
                <span class="text-[10px] font-bold text-text_secondary opacity-40 mt-1 uppercase tracking-widest">SPOT FX // {detail().symbol}</span>
              </div>
            </div>

            <div class="flex items-center gap-10">
              <div class="flex flex-col items-end">
                <span class="text-[26px] font-black font-mono text-text_primary leading-none">{fmt(detail().price, 4)}</span>
                <span class={`text-[10px] font-black mt-1 ${signColor(detail().institutional?.regularMarketChange)}`}>
                  {detail().institutional?.regularMarketChange > 0 ? '+' : ''}{fmt(detail().institutional?.regularMarketChange, 4)} ({fmtPct(detail().institutional?.regularMarketChangePercent)})
                </span>
              </div>
              <div class="h-10 w-px bg-border_main opacity-30"></div>
              <div class="flex flex-col items-end">
                <span class="text-[10px] font-black text-text_secondary/60 uppercase tracking-widest mb-1">PREV_CLOSE</span>
                <span class="text-[14px] font-black text-text_primary font-mono">{fmt(detail().institutional?.previousClose, 4)}</span>
              </div>
            </div>
          </Show>
        </div>
        ...
        <div class="flex-1 overflow-y-auto win-scroll p-6 bg-black/5">
            ...
            <div class="flex flex-col gap-6 animate-in slide-in-from-right-4 duration-500">
                ...
                <div class="col-span-12 lg:col-span-4 flex flex-col border-2 border-border_main bg-black/40 overflow-hidden">
                  ...
                  <div class="flex-1 p-5 flex flex-col gap-4 overflow-y-auto win-scroll">
                    ...
                    <div class="space-y-4 pt-2">
                      ...
                      <div class="grid grid-cols-1 gap-1">
                        <div class="flex items-center justify-between py-1.5 border-b border-border_main/10 text-[9px]">
                          ...
                          <span class="font-mono font-bold text-text_primary">{fmt(detail()?.institutional?.fiftyDayAverage, 4)}</span>
                        </div>
                        <div class="flex items-center justify-between py-1.5 border-b border-border_main/10 text-[9px]">
                          <span class="font-black text-text_secondary uppercase">MA_200_DAY</span>
                          <span class="font-mono font-bold text-text_primary">{fmt(detail()?.institutional?.twoHundredDayAverage, 4)}</span>
                        </div>
                        <div class="flex items-center justify-between py-1.5 text-[9px]">
                          ...
                          <span class="px-2 py-0.5 bg-blue-500/10 border border-blue-500/30 text-blue-400 font-black tracking-tighter uppercase">INTERBANK_FX</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="mt-auto pt-6 border-t border-border_main/20">
                      ...
                      <p class="text-[8px] text-text_secondary italic leading-tight opacity-60 uppercase">
                        Institutional feed active // Global Surveillance Network.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div class="grid grid-cols-12 gap-6">
                ...
                <div class="col-span-12 lg:col-span-4 flex flex-col border-2 border-border_main bg-black/40 h-[400px]">
                  <SectionHeader title="PRICE MATRIX" />
                          ...
                          {(row, idx) => {
                            ...
                            return (
                              <tr class="hover:bg-white/5 transition-colors">
                                ...
                                <td class="p-3 text-right text-text_accent font-black">{fmt(row.Close, 4)}</td>
                                <td class={`p-3 text-right font-black ${signColor(delta)}`}>
                                  {delta > 0 ? '▲' : delta < 0 ? '▼' : ''} {fmt(Math.abs(delta), 4)}
                                </td>
                              </tr>
                            );
                          }}
          ...
          <Show when={activeTab() === 'technical'}>
            <div class="h-full flex flex-col gap-6 animate-in fade-in duration-500 overflow-y-auto win-scroll pr-2">
              ...
              <div class="h-[600px] shrink-0 border-2 border-border_main mb-6">
        ...
        <div class="h-10 border-t border-border_main bg-bg_header/20 shrink-0 flex items-center justify-between px-6">
          <div class="flex items-center gap-6 text-[9px] font-black text-text_secondary tracking-widest">
            <div class="flex items-center gap-2 font-mono">
              <div class="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.8)]" />
              <span class="uppercase">STATUS: SYNC_CONNECTED</span>
            </div>
            <div class="flex items-center gap-2 font-mono uppercase">
              <span class="text-text_secondary opacity-40">FEED: GLOBAL_SURVEILLANCE</span>
            </div>
          </div>
