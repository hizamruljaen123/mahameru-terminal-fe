...
export default function MarketDashboard(props) {
  ...
  onMount(async () => {
    ...
    // 1. Load from preload cache instantly (no waiting)
    const cached = await loadPreloadCache();
    if (cached) {
      if (cached.crypto) setCryptoMovers(cached.crypto);
      if (cached.forex) setForexMovers(cached.forex);
      if (cached.macro) setMacroNews(cached.macro);
      if (cached.sentiment) setSentimentSummary(cached.sentiment);
      if (cached.ai) setAiVerdicts(cached.ai);
      if (cached.regime) setRegimeData(cached.regime);
      if (cached.sectorRotation) setSectorRotation(cached.sectorRotation);
      if (cached.supplyChain) setSupplyChainData(cached.supplyChain);
      setIsLoading(false); // Show cached data immediately
      console.log('[MarketDashboard] Preload cache applied instantly');
    }

    // 2. Fetch fresh data in background
    await fetchGlobalIntelligence();

    // 3. Set up refresh interval
  ...
  const fetchGlobalIntelligence = async () => {
    ...
    try {
      ...
      const [cRes, fRes, nRes, sRes] = await Promise.all([
        fetchWithRetry(`${CRYPTO_API}/api/crypto/top?top=15`, {}, { retries: 1, backoffBase: 1000 }).catch(e => { console.error("Crypto Fetch Failed:", e); return { status: 'error' }; }),
        fetchWithRetry(`${FOREX_API_BASE}/api/forex/list`, {}, { retries: 1, backoffBase: 1000 }).catch(e => { console.error("Forex Fetch Failed:", e); return { status: 'error' }; }),
        fetchWithRetry(`${DASHBOARD_API}/api/dashboard/news-preload?category=INVESTASI`, {}, { retries: 1, backoffBase: 1000 }).catch(e => { console.error("Macro Fetch Failed:", e); return { status: 'error' }; }),
        fetchWithRetry(SENTIMENT_API, {}, { retries: 1, backoffBase: 1000 }).catch(e => { console.error("Sentiment Fetch Failed:", e); return { status: 'error' }; })
      ]);
      ...
      if (cRes.status === 'success') {
        ...
        const fetchWithTimeout = (url, timeout = 15000) =>
          Promise.race([
            fetchWithRetry(url, {}, { retries: 1, backoffBase: 500 }),
      ...
      if (sRes.status === 'success') {
        freshData.sentiment = sRes.data;
        setSentimentSummary(freshData.sentiment);
      }

      // Fetch regime data
      try {
        const regimeRes = await fetchWithRetry(`${REGIME_API}/api/regime/current`, {}, { retries: 1, backoffBase: 1000 });
        if (regimeRes.status === 'success') {
          setRegimeData(regimeRes.data);
          freshData.regime = regimeRes.data;
        }
      } catch (e) {
      ...
      try {
        const rotRes = await fetchWithRetry(`${MARKET_API}/api/market/sector-rotation`, {}, { retries: 1, backoffBase: 1000 });
        if (rotRes.status === 'success') {
          const rot = rotRes.data?.rotation || rotRes.data || [];
          setSectorRotation(rot);
          freshData.sectorRotation = rot;
        }
      } catch (e) {
      ...
      try {
        const scRes = await fetchWithRetry(`${SUPPLY_CHAIN_API}/api/supply-chain/summary`, {}, { retries: 1, backoffBase: 1000 });
        if (scRes.status === 'success') {
          setSupplyChainData(scRes.data);
          freshData.supplyChain = scRes.data;
        }
      } catch (e) {
        console.warn("Supply chain fetch failed:", e);
      }

      // Persist to preload cache for next visit
      savePreloadCache(freshData);
