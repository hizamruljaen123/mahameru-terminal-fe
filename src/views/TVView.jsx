  ...
  const fetchChannels = async () => {
    ...
    try {
      ...
      if (data && Array.isArray(data.channels)) {
        ...
        console.warn("[ChannelBrowser] Unexpected response format:", data);
        setChannels([]);
        setTotalCount(0);
        setTotalPages(0);
      }
    } catch (e) {
      console.error("[ChannelBrowser] Failed to fetch channels from BE:", e);
      setChannels([]);
      setTotalCount(0);
      setTotalPages(0);
    } finally {
    ...
    fetch(`${import.meta.env.VITE_TV_API}/api/tv/sources`)
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          ...
          console.error("[TVView] Invalid sources response:", data);
          setSources([]);
        }
      })
      .catch((err) => {
        console.error("[TVView] Failed to load sources:", err);
        setSources([]);
      });
  });
